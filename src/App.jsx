  ...
  return (
    <motion.article initial={{ opacity: 0, y: 18 }} animate={{ opacity: past ? 0.46 : 1, y: 0 }} transition={{ delay: index * 0.025 }} className={`group overflow-hidden rounded-3xl border bg-zinc-950 shadow-2xl shadow-black/30 ${past ? "border-white/5 grayscale" : "border-white/10"}`}>
      <button onClick={() => onOpen(race)} className="block w-full text-left">
        ...
        <div className="border-t border-white/10 bg-zinc-950 p-5"><div className="mb-4 flex flex-wrap items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-black uppercase ${past ? "bg-zinc-700 text-zinc-300" : "bg-white text-zinc-950"}`}>{past ? "Finished" : live ? "On now" : race.status}</span>{race.major && <span className="inline-flex items-center gap-1 rounded-full bg-yellow-300 px-3 py-1 text-xs font-black uppercase text-zinc-950"><Trophy className="h-3 w-3" /> Mega</span>}<span className="rounded-full border border-white/10 px-3 py-1 text-xs font-bold uppercase text-zinc-400">{series.name}</span><span className={`rounded-full border px-3 py-1 text-xs font-black uppercase ${official ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-200" : "border-zinc-500/40 bg-zinc-800 text-zinc-300"}`}>{official ? "Official times loaded" : "Official TBC"}</span><span className="rounded-full border border-red-400/40 bg-red-500/10 px-3 py-1 text-xs font-black uppercase text-red-200">Click for sessions</span></div><p className="text-sm leading-6 text-zinc-300">{race.notes}</p></div>
      </button>
    </motion.article>
  );
}
